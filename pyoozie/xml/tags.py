...
REGEX_IDENTIFIER = '^[a-zA-Z_][\-_a-zA-Z0-9]{0,38}$'
...
class IdentifierTooLongError(AssertionError):
    ...
    def __init__(self, identifier):
        AssertionError.__init__(self, "Identifier must be less than "
                                "{max_length} chars long, '{identifier}' is "
                                "{length}".format(
...
class Parameters(_PropertyList):
    ...
    Allows one to specify properties that can be reused in actions. "Properties
    that are a valid Java identifier, [A-Za-z_][0-9A-Za-z_]* , are available as
    '${NAME}' variables within the workflow definition."

    "Properties that are not valid Java Identifier, for example 'job.tracker',
    are available via the String wf:conf(String name) function. Valid
    identifier properties are available via this function as well." """
...
class Configuration(_PropertyList):
    """Coordinator job submission, workflow, workflow action
       configuration XML."""
...
class Shell(Xml):
    ...
    def __init__(self, exec_command, job_tracker=None, name_node=None,
                 prepares=None, job_xml_files=None,
                 configuration=None, arguments=None,
                 env_vars=None, files=None, archives=None,
    ...
    def _xml(self, doc, tag, text):
        with tag(self.xml_tag, xmlns='uri:oozie:shell-action:0.3'):
            ...
            if self.prepares:
                raise NotImplementedError("Shell action's prepares has not "
...
class SubWorkflow(Xml):
    ...
    An Oozie sub-workflow is an "action [that] runs a child workflow job [...].
    The parent workflow job will wait until the child workflow job has
    completed." """

    def __init__(self, app_path, propagate_configuration=True,
                 configuration=None):
...
class GlobalConfiguration(Xml):
    ...
    name-node declarations for each action. [...] The global section may
    contain the job-xml, configuration, job-tracker, or name-node that the user
    would like to set for every action.  If a user then redefines one of these
    in a specific action node, Oozie will update [sic] use the specific
    declaration instead of the global one for that action."

    "The job-xml element, if present, must refer to a Hadoop JobConf job.xml
    file bundled in the workflow application." """

    def __init__(self, job_tracker='', name_node='', job_xml_files=None,
                 configuration=None):
...
class Email(Xml):
    ...
    def __init__(self, to, subject, body, cc=None, bcc=None, content_type=None,
                 attachments=None):
    ...
    def _xml(self, doc, tag, text):
        def format_list(emails):
            return ','.join(sorted(emails)) if hasattr(emails, '__iter__') \
                else emails
